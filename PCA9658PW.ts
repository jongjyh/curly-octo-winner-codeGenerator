...
namespace Motor {
    ...
    *
    * */
    function set_Motor_Pwm(channel: number, on: number, off: number): void {
        if (channel < 0 || channel > 15)
            return;
    ...
    */
    ...
    //% weight=85
    //% pulse.min=0 pulse.max=4095
    export function MotorPulse(channel: number,pulse: number): void {
        if (!initialized) {
            initPCA9685();
        }
        // 50hz: 20,000 us
